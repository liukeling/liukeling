...
@SuppressLint("HandlerLeak") public class Chat extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(obj instanceof user){
			...
			Toast.makeText(this, "我不知道Ta是谁", 1).show();
			this.finish();
		}
		...
		message.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable s) {
				...
				if(message.getText().toString() != null && !"".equals(message.getText().toString())){
					...
					sendButton.setBackgroundResource(R.drawable.sendbutton_chat);
				}else{
					sendButton.setClickable(false);
					sendButton.setBackgroundResource(R.drawable.sendbutton_chat1);
				}
			}
		});
